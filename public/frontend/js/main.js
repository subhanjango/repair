...
function windowLoadInit() {
	...
	if (jQuery().flexslider) {
		...
		var $teamSlider = jQuery(".team_flexslider");
		$teamSlider.flexslider({
			...
			prevText: "",
			nextText: "",
			controlsContainer: ".team_flexslider .slide_description",
			smoothHeight: false,
			slideshowSpeed:10000,
			animationSpeed:600,
			start: function( slider ) {
				slider.find('.to_animate').css({'visibility': 'hidden'});
				slider.find('.flex-active-slide .to_animate').each(function(index){
					var self = jQuery(this);
					var animationClass = !self.data('animation') ? 'fadeInLeftMedium' : self.data('animation');
					setTimeout(function(){
						self.addClass("animated "+animationClass);
					}, index*200);
				});
			...
			before: function( slider ) {
				slider.find('.flex-active-slide').find('.to_animate').each(function(index){
					var self = jQuery(this);
					var animationClass = !self.data('animation') ? 'fadeInLeftMedium' : self.data('animation');
					var animation2Class = !self.data('animation2') ? 'fadeInRightMediumReverse' : self.data('animation2');
					self.removeClass('animated ' + animationClass);
					self.addClass("animated "+ animation2Class);
				});
			},
			after :function( slider ){
				slider.find('.to_animate').each(function(index){
					var self = jQuery(this);
					var animationClass = !self.data('animation') ? 'fadeInLeftMedium' : self.data('animation');
					var animation2Class = !self.data('animation2') ? 'fadeInRightMediumReverse' : self.data('animation2');
					self.removeClass('animated ' + animationClass + ' ' + animation2Class);
				})
				slider.find('.flex-active-slide .to_animate').each(function(index){
					var self = jQuery(this);
					var animationClass = !self.data('animation') ? 'fadeInLeftMedium' : self.data('animation');
		...
		jQuery(".flexslider").each(function(index){
		    ...
		    if (($currentSlider.find('.flex-active-slide').length) || $currentSlider.hasClass('team_flexslider')) {
		     return;
		    }
			$currentSlider.flexslider({
				animation: "fade",
	...
	if (jQuery().appear) {
		jQuery('.to_animate').not('.flexslider .to_animate').appear();
		jQuery('.to_animate').not('.flexslider .to_animate').filter(':appeared').each(function(index){
			var self = jQuery(this);
	...
	jQuery('.embed-placeholder').each(function(){
		jQuery(this).on('click', function(e) {
			e.preventDefault();
			jQuery(this).replaceWith('<iframe class="embed-responsive-item" src="'+ jQuery(this).attr('href') + '?rel=0&enablejsapi=1&autoplay=1"'+ ' allowscriptaccess="always"' + '"></iframe>');
		});
	});
	...
	setTimeout(function(){
		jQuery(".twentytwenty-container").twentytwenty({
		  default_offset_pct: 0.53, // How much of the before image is visible when the page loads
